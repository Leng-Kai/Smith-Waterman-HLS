...
void store_diagonal(int directions_index, ap_uint<K> *direction_matrix_g, ap_uint<K> compressed_diag[1]) {

	memcpy(direction_matrix_g + directions_index, compressed_diag, K/8);

}

void update_database(ap_uint<K> *database, ap_uint<K> *shift_db, int num_diagonals){
...
void calculate_diagonal(int num_diagonals, ap_uint<K> * string1, ap_uint<K> * string2, int northwest[N + 1], int north[N + 1], int west[N + 1], int directions_index, ap_uint<K> compressed_diag[], ap_uint<K> shift_db[], ap_uint<K> *direction_matrix_g){
...
//			directionDiagonal[index] = CENTER;
	...
	memcpy((ap_int<K>*) ((direction_matrix_g + num_diagonals)), compressed_diag,K/8);
...
{
#pragma HLS INTERFACE m_axi port=string1_g offset=slave bundle=gmem0 depth=32
#pragma HLS INTERFACE m_axi port=string2_g offset=slave bundle=gmem0 depth=32
#pragma HLS INTERFACE m_axi port=direction_matrix_g offset=slave bundle=gmem1 depth=160
...
#pragma HLS INTERFACE s_axilite port=return bundle=control
	...
	ap_uint<K> string1[N / NUM_ELEM  + 1];
//#pragma HLS ARRAY_PARTITION variable=string1 complete dim=1
	ap_uint<K> string2[(DATABASE_SIZE)/ NUM_ELEM + 1];
...
//#pragma HLS ARRAY_PARTITION variable=direction_matrix complete dim=1
	...
	memcpy(string1, string1_g, (N/NUM_ELEM + 1) * K/8);
	memcpy(string2, string2_g, ((DATABASE_SIZE) / NUM_ELEM + 1) * K/8);
...
*/
	...
	ap_uint<K> compressed_diag[1];
	// ap_uint<512> shift_db[N/NUM_ELEM];
	ap_uint<K> shift_db[N/NUM_ELEM];
...
#pragma HLS PIPELINE
	...
	// memcpy(shift_db, string2_g, (N/NUM_ELEM) * 64);
	...
	// memcpy(shift_db, string2, 64*N/NUM_ELEM);
