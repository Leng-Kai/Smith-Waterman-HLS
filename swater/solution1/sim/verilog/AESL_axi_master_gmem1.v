...
module AESL_axi_master_gmem1 (
    ...
    TRAN_gmem1_BID,
    TRAN_gmem1_BUSER,
    TRAN_gmem1_direction_matrix_g_V,
    ready,
    done
    );

//------------------------Parameter----------------------
`define TV_IN_gmem1 "../tv/cdatafile/c.compute_matrices.autotvin_gmem1.dat"
`define TV_OUT_gmem1 "../tv/rtldatafile/rtl.compute_matrices.autotvout_gmem1.dat"
 parameter gmem1_ADDR_BITWIDTH = 32'd 32;
 parameter gmem1_AWUSER_BITWIDTH = 32'd 1;
 parameter gmem1_DATA_BITWIDTH = 32'd 64;
...
parameter gmem1_C_DATA_BITWIDTH = 32'd 64;
parameter gmem1_mem_depth = 32'd 160;
...
output [32 - 1 : 0] TRAN_gmem1_direction_matrix_g_V;
input ready;
input done;
...
reg RLAST_tmp = 0;
reg RVALID_tmp = 0;
reg [32 - 1 : 0] direction_matrix_g_V = 0;
...
assign    TRAN_gmem1_direction_matrix_g_V = direction_matrix_g_V;
...
initial begin : initialize_offset
  ...
  direction_matrix_g_V <= 0 * DATA_byte_num;
end
    ...
    while (1) begin
        if (FIFO_AW_req_empty === 1 || BRESP_counter == 32) begin
            ...
            if (FIFO_AW_req_ADDR_tmp/data_byte_size > gmem1_mem_depth) begin
                $display ("C:/WorkZone/ZedBoard/swater_4/solution1/sim/verilog/AESL_axi_master_gmem1.v: Write request address %d exceed AXI master gmem1 array depth: %d",FIFO_AW_req_ADDR_tmp/data_byte_size, gmem1_mem_depth); 
                $finish;
            end
    ...
    while (1) begin
        ...
        if (FIFO_AR_req_empty) begin
            ...
            if (FIFO_AR_req_ADDR_tmp/data_byte_size > gmem1_mem_depth) begin
                $display ("C:/WorkZone/ZedBoard/swater_4/solution1/sim/verilog/AESL_axi_master_gmem1.v: Read request address %d exceed AXI master gmem1 array depth: %d",FIFO_AR_req_ADDR_tmp/data_byte_size, gmem1_mem_depth); 
                $finish;
            end
...
initial begin : write_file_proc 
  ...
  while(1) begin 
      ...
      if (factor == 2) begin
          if ((gmem1_mem_depth - 1) % factor == 0) begin
              case(mem_page)
                  0 : $fdisplay(fp,"0x%x",gmem1_mem_0[gmem1_mem_depth/factor][15:0]);
                  1 : $fdisplay(fp,"0x%x",gmem1_mem_1[gmem1_mem_depth/factor][15:0]);
                  2 : $fdisplay(fp,"0x%x",gmem1_mem_2[gmem1_mem_depth/factor][15:0]);
                  default: $display("The page_num of write file is not valid!");
              endcase
          end
      end
